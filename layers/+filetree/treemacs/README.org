...
- Location awareness: commands like ~find-file~ or ~magit-status~ will use the
  location of the node at point (with =$E_HOME= as fallback).
...
| ~SPC 0~     | Open treemacs in the current directory. When not visiting a file use $E_HOME as fallback. With a prefix arg manually select the root instead.    |
